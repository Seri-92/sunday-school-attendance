...
export type SummaryTrendChartRow = SummaryTrendPoint & {
  elementaryLabel: string;
  juniorHighLabel: string;
  totalLabel: string;
};
...
export function buildSummaryTrendChartRows(
  points: SummaryTrendPoint[],
): SummaryTrendChartRow[] {
  return points.map((point) => ({
    ...point,
    elementaryLabel: `${formatAttendanceAverageCount(point.elementaryCount)} 名`,
    juniorHighLabel: `${formatAttendanceAverageCount(point.juniorHighCount)} 名`,
    totalLabel: `${formatAttendanceAverageCount(point.totalCount)} 名`,
  }));
}
