...
日曜学校の出席管理アプリです。Phase 1 では Neon + PostgreSQL + Drizzle の DB 基盤と seed を整備しています。
...
`role`、`grade_code`、`assignment_type` は DB 上で型を固定しています。出席 `status` は未確定要件が残っているため、現時点では文字列として保持します。
